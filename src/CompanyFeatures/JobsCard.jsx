...
const JobsCard = () => {
  ...
  if (!categoriesID || categoriesID.length === 0) {
    return <div className="text-center mt-10 text-red-500">No Company Card Found.</div>;
  }
        ...
        {categoriesID.map(company => (
          <div className='w-11/12 mx-auto mt-9' key={company.id}>
            {company.jobs.map(job => (
              <div key={job.id} className="card w-full bg-base-100 shadow-xl rounded-xl p-6 mb-4">
                ...
                <figure className="mb-4">
                  <img src={devTable} alt="Job banner" className="w-full h-64 object-cover rounded-xl" />
                </figure>
