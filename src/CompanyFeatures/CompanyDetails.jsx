...
const CompanyDetails = () => {
  ...
  return (
    <div className="my-15">
      ...
      {/* Grid Container */}
    { detail && <div className="grid grid-cols-1 md:grid-cols-2 gap-11 w-11/12 mx-auto py-6">
        {
        detail.map((company) => (
          <div
            key={company.id}  
          ...
          >
            <div className="flex justify-around py-5 items-center">
              <img 
                src={company.logo}
                alt=""
                className="w-54 h-54 object-cover my-2 rounded-full"
              />

              <div className="text-left ml-4 lh-20">
                <h2 className="text-3xl mb-4 font-semibold">{company.name}</h2>
